# -*- coding: utf-8 -*-
"""涨停复盘报告生成（占位符替换版）：今日(9-16) vs 昨日(9-15) + 资金运动分析"""
...
S = json.load(open(os.path.join(OUT, "zt_stats.json"), encoding="utf-8"))
B = json.load(open(os.path.join(OUT, "zt_review_20260916.json"), encoding="utf-8"))
r0, r1 = S["r0"], S["r1"]
perf = S["perf"]
D0, D1 = "20260916", "20260915"
...
# ---- 三日情绪 ----
days = [
    {"d": "9/14", "zt": s1["zt_prev"], "zb": s1["zb_prev"], "dt": s1["dt_prev"]},
    {"d": "9/15", "zt": s1["zt"], "zb": s1["zb"], "dt": s1["dt"]},
    {"d": "9/16", "zt": s0["zt"], "zb": s0["zb"], "dt": s0["dt"]},
]
for x in days:
...
PREV_IDX_PCT = {"上证指数": -0.54, "深证成指": -0.72, "创业板指": -1.15, "科创50": 1.55}

amt_today = ixr["上证指数"]["amt"] + ixr["深证成指"]["amt"]
amt_prev = 7639.8 + 8487.3
amt_pct = (amt_today / amt_prev - 1) * 100
kc = ixr.get("科创50", {})
kc_amt_pct = (kc.get("amt", 0) / 725.0 - 1) * 100
kc_prev_amt = 725.0

# ---- 行业迁移 ----
hy0, hy1 = S["hy0"], S["hy1"]
hy_keys = sorted(set(list(hy0) + list(hy1)), key=lambda k: -(hy0.get(k, 0) * 2 + hy1.get(k, 0)))
...
perf_sorted = sorted(perf, key=lambda x: -(x["pct"] or -99))
top3 = perf_sorted[:3]
bot3 = perf_sorted[-3:]
...
fp = os.path.join(REP, "涨停复盘对比-20260916.html")
