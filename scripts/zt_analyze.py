# -*- coding: utf-8 -*-
"""涨停复盘分析：情绪温度计 / 连板梯队 / 题材归因 / 晋级率 / 资金运动"""
import json, os, sys, re, time, datetime, collections
from statistics import mean, median

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
OUT = os.path.join(ROOT, "out")
D0, D1 = "20260916", "20260915"   # 今日 / 昨日
D2 = "20260914"                    # 前日

B = json.load(open(os.path.join(OUT, "zt_review_20260916.json"), encoding="utf-8"))
D = B["dates"]
...
def build(date):
    ...
    for it in ths:
        ...
        rows.append({
            ...
            "lbc": lbc if lbc is not None else None,
...
def ladder(rows):
    """连板梯队：用东财 lbc 为主，缺失用 high_days 文本"""
    def lv(r):
        if r["lbc"]:
            return int(r["lbc"])
        s = r["high_days"]
        ...
        if s == "首板": return 1
        return 1
    cnt = collections.Counter(lv(r) for r in rows)
    return cnt

def timeslot(hhmm):
    if not hhmm: return "未知"
    t = hhmm[:5].replace(":", "")
    try: v = int(t)
    except Exception: return "未知"
    if v <= 925: return "竞价/秒板"
    if v <= 1000: return "开盘半小时"
    if v <= 1130: return "上午盘中"
    if v <= 1430: return "午后盘中"
    if v <= 1500: return "尾盘"
    return "未知"
...
print("=" * 70)
senti = {}
for d, tag in [(D0, "今日9/16"), (D1, "昨日9/15"), (D2, "前日9/14")]:
    rec = D.get(d) or {}
    ...
    senti[d] = {
        ...
        "ladder": dict(lad),
        "max_board": max([k for k in lad if lad[k] > 0], default=0) if lad else None,
    ...
    print(f"\n[{tag}]  涨停={len(info)}  炸板={zb}  封板率={(len(info)/(len(info)+zb)*100 if (len(info)+zb) else 0):.1f}%  跌停={dt}")
...
print("【三】连板梯队明细（今日）")
print("=" * 70)
r0 = build(D0); r1 = build(D1)
for r in sorted(r0, key=lambda x: -(x["lbc"] or 0)):
    if (r["lbc"] or 1) >= 2:
        print(f"  {r['lbc']}板 {r['code']} {r['name']:<8} {r['fbt']} 封单{(r['fund'] or 0)/1e8:.2f}亿 额{(r['amount'] or 0)/1e8:.2f}亿 [{r['high_days']}] {r['hybk']} | {r['reason']}")

print()
print("=" * 70)
print("【四】今日涨停题材归因（原始 reason_type 去重）")
print("=" * 70)
...
allhy = sorted(set(list(hy0) + list(hy1)), key=lambda k: -(hy0.get(k, 0) + hy1.get(k, 0)))
for k in allhy[:40]:
...
order = ["竞价/秒板", "开盘半小时", "上午盘中", "午后盘中", "尾盘", "未知"]
for k in order:
...
for k in ["昨日首板", "昨日连板"]:
    ...
    grp_stat[k] = {"n": len(g), "adv": ga / len(g) * 100, "mean": mean(gp),
                   "median": median(gp), "neg": sum(1 for p in gp if p < 0)}
...
print(f"  一字板: {sum(1 for r in r0 if '一字' in r['limit_up_type'])}  T字板: {sum(1 for r in r0 if 'T字' in r['limit_up_type'])}  换手板: {sum(1 for r in r0 if '换手' in r['limit_up_type'])}")
...
json.dump({"senti": senti, "hy0": dict(hy0), "hy1": dict(hy1),
           "perf": rows_perf, "r0": r0, "r1": r1,
           "timeslot": {"today": dict(ts0), "yesterday": dict(ts1)}},
          open(os.path.join(OUT, "zt_stats.json"), "w", encoding="utf-8"), ensure_ascii=False, indent=1)
print("\nsaved -> out/zt_stats.json")
