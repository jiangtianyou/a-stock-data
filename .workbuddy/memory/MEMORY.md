...
- **「收缩式锁仓」= 参与度与承接力背离（2026-09-24 标本）**：指数全线中阴、成交 -6.3%，但涨停 51→51 持平、炸板 26→10、封板率 66.2%→83.6%。**关键是否决条件：触及涨停家数 77→61（-21%）**，说明愿意冲板的钱少了，只是因为没有换手所以封得住。三条闭合证据：涨停股合计成交 **-15%**、单只中位成交 **-15%**、而封单中位数 **+13%**。**遇「指数跌 + 缩量 + 封板率上升」先算触及涨停家数**，否则会把"收缩"误写成"修复"。同型辅助判据：**炸板数骤降要看炸板池市值结构**（09-23 池内是万科Ａ/天山股份/科华数据/超声电子等中大盘，09-24 只剩先导基电一只过百亿）→ 减少的是"中大盘冲板"而非承接变强。
- **成交额环比的"镜像反转"是最强的次日复核工具（2026-09-23 ↔ 09-24 连续验证）**：09-22 结论"增量是配置盘"（科创50 +27.2%、上证50 +17.8%）→ 09-23 这两者缩得最狠（-33.8% / -34.0%），证伪；09-23 结论"权重先撤"→ 09-24 变成 **上证50 +5.7%（唯一放量）**、小盘缩得最狠（中证1000 -11.2%、国证2000 -9.0%、北证50 -12.9%）。**把"各指数成交额环比"当作每日必看的第一张表，可以自动识别前一日结论是否被证伪。**
- **东财行业标签在题材市里失效（2026-09-24 标本）**：51 只涨停分散在 32 个东财行业、最大集群仅占 11.8%；「通用设备」6→6 家**家数持平但成分只留存 1 只（轮换率 83%）**。**主线归因必须用 `reason_type` 关键词聚类，行业标签只作辅助**；且家数持平时必须用代码集合交集算轮换率，不能凭家数下"延续"结论。
- **区域题材（海峡两岸/福建，2026-09-24 标本）**：10 只（占涨停 20%），横跨港口物流/工程机械/林业/水泥/电力/智慧城市/AI医疗/纺织/AI营销，唯一共同点是地域与"国资/资产重组"关键词，**全部低价小盘、无业绩弹性、无高度（9 只首板 + 1 只 2 板）**。**读法：与「某字辈」同性质——主线真空期的替代品**；持续性看政策落地与首日封单厚度（本次封单 9.31 亿中平潭发展一只占 3.57 亿，分布极不均衡）。
- **「收在涨停价」不等于被计入涨停家数（2026-09-22 / 09-24 两次实测）**：瑞芯微 +10.00%、三羊马 +10.01% 收盘价均等于涨停价，但同花顺与东财**涨停池都不收录**，只进东财炸板池（推测按"收盘是否有封单"筛选）。家数一律取两源汇总口径，报告中单列披露并计入"回封"。
